"""The seeded demo project — "The Cartographer's Daughter" by Mira Halden.

Ported verbatim from the renderer's old `src/renderer/src/domain/seed.js`
(removed in the 2026-06-20 cross-app convergence, item #7) so the demo lives
server-side like JustVoice's `demo_projects.py` — the thin-client rule: seed /
demo data comes from the server, never the client. `demo_book_snapshot()`
returns the exact snapshot shape `book_io.decompose` consumes (the same shape
`book_io.assemble` emits and the renderer's `exportSnapshot()` produces). All
names and places are invented for the demo.

Scenes carry no `id` in the source map; the renderer minted `scn_{chId}_{i+1}`
at boot, so we mint the same ids here (the strand beats reference them, e.g.
`scn_ch4_1`).
"""

from __future__ import annotations

# Fixed id so a reset-then-reseed produces the same project, and so the
# `demoSeeded` gate is unambiguous.
DEMO_PROJECT_ID = "prj_demo_cartographer"
...
_PROJECT = {
    ...
    "premise": (
        "A reclusive mapmaker's daughter inherits a ledger of places that no "
        "longer exist — and must decide which of them are worth bringing back."
    ),
    "coverImage": None,
}

_STRANDS = [
    {"id": "s1", "name": "Inheritance", "color": "oklch(0.82 0.08 75)", "status": "open",
     "blurb": "Elen's reckoning with what her father left behind — and whether to honour it.", "beats": []},
    {"id": "s2", "name": "The Ledger", "color": "oklch(0.78 0.06 200)", "status": "open",
     "blurb": "The book itself: who has read it, who has added to it, what its last twelve pages mean.",
     "beats": [
         {"id": "b_s2_1", "chapterId": "ch4", "sceneId": "scn_ch4_1", "label": "Inciting",
          "note": "Elen finds the bearing that doesn't match any chart she owns."},
         {"id": "b_s2_2", "chapterId": "ch9", "sceneId": "scn_ch9_3", "label": "Midpoint",
          "note": "The hand on the last twelve pages turns out to be someone she knows."},
     ]},
    {"id": "s3", "name": "Old Ports", "color": "oklch(0.74 0.07 270)", "status": "open",
     "blurb": "The harbours and coastlines the ledger names — places that have moved, vanished, or never existed.", "beats": []},
    {"id": "s4", "name": "Mother's silence", "color": "oklch(0.78 0.07 25)", "status": "open",
     "blurb": "What Petra refuses to say, and what Elen finally asks.",
     "beats": [
         {"id": "b_s4_1", "chapterId": "ch8", "sceneId": "scn_ch8_2", "label": "Refusal",
          "note": "Petra walks out of the room rather than answer the question."},
     ]},
    {"id": "s5", "name": "What returns", "color": "oklch(0.78 0.06 120)", "status": "open",
     "blurb": "Brackish Cove, the tide, and the question of which inheritances can be brought back.", "beats": []},
]

_CHARACTERS = [
    {"id": "c1", "main": True, "name": "Elen Vael", "role": "Protagonist", "age": 31,
     "oneLiner": "Cartographer's daughter; archivist by trade; quietly losing faith in maps."},
    {"id": "c2", "main": True, "name": "Idris Vael", "role": "Father (dec.)", "age": 67,
     "oneLiner": "Drew coastlines that disagreed with the sea. Left behind a ledger."},
    {"id": "c3", "main": True, "name": "June Asari", "role": "Friend / cipher", "age": 30,
     "oneLiner": "A printer who reads ink the way most people read faces."},
    {"id": "c4", "main": True, "name": "Halvard Renn", "role": "Antagonist?", "age": 52,
     "oneLiner": "Catalogues what is. Believes the ledger is a forgery worth burning."},
    {"id": "c5", "main": False, "name": "Petra Lange", "role": "Aunt", "age": 64,
     "oneLiner": "Remembers a coast that her sister claimed never existed."},
    {"id": "c6", "main": False, "name": "Tomas", "role": "Boy on the pier", "age": 9,
     "oneLiner": "Has been waiting a long time for someone to ask the right question."},
    {"id": "c7", "main": False, "name": "Mrs. Oren", "role": "Neighbor", "age": 71,
     "oneLiner": "Speaks in the present tense about people who are gone."},
    {"id": "c8", "main": False, "name": "The Surveyor", "role": "Unknown", "age": None,
     "oneLiner": "Initials only. Three letters in the ledger's last column."},
]
...
_CHARACTER_EXTRAS = {
    "c1": {
        ...
        "backstory": (
            "Born 1995 in a storm year. Mother left when she was eight, never explained. "
            "Father raised her in Halden House on the cliff road. Trained as an archivist "
            "(two years; never finished). Returned to care for Idris in his last illness and "
            "stayed. Inherited the house, the theodolite, and the ledger in March 2026."
        ),
    ...
    "c4": {
        "voice": {"accent": "RP, slightly clipped.", "vocabulary": "Civil-service phrasings.",
                  "tic": "Pauses before disagreeing, as if checking a list.",
                  "sample": "I am not asking you to give up the book. I am asking you to consider what it is."},
        "arc": {"start": "Believes Idris was a kindly fraud.", "midpoint": "Realises Elen is not her father.",
                "end": "Walks away with the question."},
        "motivation": {"want": "Remove the ledger from circulation.", "need": "To be wrong once.",
                       "lie": "That his catalogue keeps the coast honest.",
                       "truth": "That an honest coast may require dishonest entries."},
    ...
    "c3": {
        "voice": {"accent": "Mancunian softened by twenty years away from it.",
                  "vocabulary": "Printer's vernacular.", "tic": "Manages objects as if they had feelings.",
                  "sample": "I read the last twelve pages on the train. Twice."},
...
_PARTS = [
    {"id": "p1", "title": "The Inheritance", "chapters": [
        {"id": "ch1", "num": 1, "title": "What the door remembers", "words": 3120, "status": "done", "strands": ["s1"]},
        {"id": "ch2", "num": 2, "title": "An inventory in two hands", "words": 2840, "status": "done", "strands": ["s1", "s2", "s4"]},
        {"id": "ch3", "num": 3, "title": "June, with a printer's apron", "words": 2210, "status": "revise", "strands": ["s1", "s2"]},
        {"id": "ch4", "num": 4, "title": "A bearing no one recognizes", "words": 3640, "status": "done", "strands": ["s2", "s3"]},
    ]},
    {"id": "p2", "title": "The Ledger", "chapters": [
        {"id": "ch5", "num": 5, "title": "Old Harbor 7", "words": 3010, "status": "done", "strands": ["s3"]},
        {"id": "ch6", "num": 6, "title": "Renn keeps a list", "words": 2480, "status": "draft", "strands": ["s2"]},
        {"id": "ch7", "num": 7, "title": "Brackish Cove, at low tide", "words": 3995, "status": "draft", "strands": ["s3", "s5"]},
        {"id": "ch8", "num": 8, "title": "Petra refuses the chair", "words": 1820, "status": "revise", "strands": ["s4"]},
        {"id": "ch9", "num": 9, "title": "The hand that wrote the last twelve pages", "words": 2640, "status": "draft", "strands": ["s2"]},
    ]},
    {"id": "p3", "title": "What Returns", "chapters": [
        {"id": "ch10", "num": 10, "title": "St. Eira's, between tides", "words": 2380, "status": "draft", "strands": ["s5", "s3"]},
        {"id": "ch11", "num": 11, "title": "Tomas, who is older than he is", "words": 1840, "status": "todo", "strands": ["s5"]},
        {"id": "ch12", "num": 12, "title": "The unsent letter", "words": 1640, "status": "todo", "strands": ["s4", "s5"]},
        {"id": "ch13", "num": 13, "title": "A bearing, taken twice", "words": 950, "status": "todo", "strands": ["s5", "s3", "s2", "s1"]},
    ]},
]

# Per-chapter scene records, keyed by chapter id. Each entry becomes one scene;
# the builder mints scn_{chId}_{i+1} ids (1-based) and fills missing link arrays.
_SCENES = {
    "ch1": [
...
# "setting"). Each event { id, when, title, note }; `when` is a datetime-local
# string the timeline view sorts on.
_EVENTS = {
    "setting": [
...
_ARCHITECTURE = {
    "premise": {"id": "premise", "title": "Premise", "blurb": "The single sentence the novel can be reduced to.", "status": "done", "words": 184,
                "body": "A reclusive cartographer's daughter inherits a ledger of places that no longer exist — and must decide which of them are worth bringing back."},
    "fabula": {"id": "fabula", "title": "Fabula", "blurb": "The story as it happens in time.", "status": "draft", "words": 1240,
               "body": "Part One — The Inheritance.\n\nPart Two — The Ledger.\n\nPart Three — What Returns."},
    "setting": {"id": "setting", "title": "Setting", "blurb": "The world the book occupies.", "status": "draft", "words": 920,
                "body": "A north-facing coastline. Late spring into early autumn, 2026."},
}
...
_WORLDBUILDING = [
    {"id": "wb1", "category": "geography", "title": "The North Coast", "tags": ["setting"], "status": "done", "words": 540,
     "summary": "The coastline the novel takes place on.",
     "body": "The coastline runs roughly north-east from the county town to Tern Head — about forty miles of cliff and pebble.", "related": ["wb2"]},
    {"id": "wb2", "category": "geography", "title": "Brackish Cove", "tags": ["disputed"], "status": "draft", "words": 230,
     "summary": "A place whose existence is the novel's central question.",
     "body": "Drawn twice on Idris's maps, ten years apart, in two slightly different places.", "related": ["wb1"]},
    {"id": "wb3", "category": "history", "title": "The 1881 Ordnance Survey", "tags": ["maps"], "status": "done", "words": 420,
     "summary": "The first authoritative map of the coast.",
     "body": "First-edition six-inch survey, walked by a team of three over two summers."},
    {"id": "wb4", "category": "factions", "title": "Catalogers and restorers", "tags": ["theme"], "status": "draft", "words": 380,
     "summary": "The novel's central argument.",
     "body": "The catalogers believe a coastline is whatever the last authoritative survey said it is. The restorers believe a coastline is an argument the sea is having with itself."},
    {"id": "wb5", "category": "lore", "title": "The legend of vanishing places", "tags": ["myth"], "status": "draft", "words": 410,
     "summary": "Local folklore about coastal sites that come and go.",
     "body": "Variants of the same story are told in all three villages."},
]

# Status palette the demo's chapters/strands reference. Mirrors the renderer's
# DEFAULT_STATUSES so the demo opens with the same labelled colours.
_STATUSES = [
...
def demo_book_snapshot() -> dict:
    """The full demo book in the snapshot shape `book_io.decompose` consumes."""
    scenes: dict[str, list] = {}
    for ch_id, scene_list in _SCENES.items():
        out = []
        for i, scn in enumerate(scene_list):
            out.append({
                "id": f"scn_{ch_id}_{i + 1}",
                "title": scn.get("title", ""),
                "body": scn.get("body", ""),
                "characters": list(scn.get("characters", [])),
                "locations": list(scn.get("locations", [])),
                "objects": list(scn.get("objects", [])),
                "strands": list(scn.get("strands", [])),
            })
        scenes[ch_id] = out

    return {
        "project": dict(_PROJECT),
        "parts": [{**p, "chapters": [dict(c) for c in p["chapters"]]} for p in _PARTS],
        "scenes": scenes,
        "characters": [dict(c) for c in _CHARACTERS],
        "characterExtras": _CHARACTER_EXTRAS,
        "locations": [dict(x) for x in _LOCATIONS],
        "objects": [dict(x) for x in _OBJECTS],
        "groups": _GROUPS,
        "strands": _STRANDS,
        "notes": [dict(n) for n in _NOTES],
        "architecture": _ARCHITECTURE,
        "worldbuilding": _WORLDBUILDING,
        "worldbuildingCategories": _WORLDBUILDING_CATEGORIES,
