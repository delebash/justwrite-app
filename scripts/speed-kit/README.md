...
4. Run `run-bench.ps1` — PHASE 1: the 16-combo matrix (models x ngl 99/0 x
   ...
   n_cpu_moe sweep on the MoE (ngl 99 / fa 0 / ub 512; ncmoe 0..48). Hours on
   slow machines; RESUMABLE — rerun skips finished combos, failed combos retry.
   If PowerShell blocks scripts: `powershell -ExecutionPolicy Bypass -File .\<script>`
5. Copy back `results.jsonl` + `bench-log.txt` + `detect-facts.txt` into
   `bench-results/<machine>/kit/` and commit.
