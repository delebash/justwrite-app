# Worldbuilding and Architecture
...
The Setting document also has an **Events** button that opens the world-level chronological event log (see [Plot and time](plot-and-time.md) for how events work).
...
- A magic system: rules, costs, limits, lineage
- The history of a kingdom or institution
- An invented language: grammar, vocabulary, dialects
- A calendar system, a religion, a political structure
- Mythology, prophecy texts, song lyrics, in-world documents
- The economy, the technology, the medical practices
...
Both flow through to the sidebar's Worldbuilding tree, so a long article list stays visually scannable.

### The article list

When no article is selected, you see a sortable, searchable table:

- **Search bar** — searches title, summary, and tags.
- **Facet filter chips** — three rows of filter buttons: Category (single-select), Status (single-select), Tags (multi-select, ANY-match). Filters combine. A "Clear filters" button resets all of them.
- **Sortable columns** — Title (with a teaser of the summary), Category (colour-coded chip), Tags, Status (colour-coded), Words (article body word count).
- **Pagination** — 20 rows per page by default.

The result count badge shows "**N of total**" so you always know how many articles your filter is showing.

### The article editor

When you click into an article:

- **Breadcrumb** showing the category, linking back to the list.
- **Title** — inline-editable.
- **Status selector** — same statuses as chapters and characters.
...
The combination of categories + tags + status gives you three independent ways to slice: "show me all draft-status magic articles tagged 'act three'." That kind of cross-cut is the reason both systems exist.

### Status on articles

Worldbuilding articles use the same status palette as chapters and characters — Draft, Revise, Done, To-do. Use it to track which articles are written, which need more research, and which are still placeholders.

---

## Worldbuilding vs. Architecture — which to use
...
Each of these has a clear home. None of it would have fit in a Character or Location sheet — it's all about systems, not individuals.

---

## See also

- **[Story bible](story-bible.md)** — Characters, Locations, Objects (each with their own Description editor)
- **[Plot and time](plot-and-time.md)** — Narrative strands, events, Timeline
- **[Writing](writing.md)** — the manuscript editor uses the same rich-text editor as Worldbuilding articles
