...
> **⚠ Provenance correction (2026-07-16):** "per the user … SAFETY CAP" above misattributes the call. Actual provenance: during this live on-box session the user pasted per-request results and Claude examined the thinking token counts — the "thinking loop" conclusion was Claude's diagnosis from that data, jointly accepted; the verification itself was never separately written down. **The loop stands as verified.** Two caveats on the numbers: the rb2048 "starved the answer" datum rode the probe's own `n_predict 2500` (the app sends no max_tokens, so in-app the answer would not starve), and the "budget message forces the answer" mechanism was retired 2026-07-14 (U2-T4 — the launch flag is gone; the budget is per-request JSON now). The cap-as-clamp design was superseded 2026-07-16 by the user's house-layering decision — see `just-llm-runner/docs/plans/2026-07-16-reasoning-budget-house-layering.md`.
