...
## Current state (2026-07-03) — **GGUF-grounded model layer, Phase 1**: SAMPLER SCOPE EXPANDED — decisions in progress (implementation NOT STARTED)
...
>
> **OPEN decisions (pending — do NOT build the sampler part until resolved + "go"):** **[OPEN-A]** catalog ↔ recommendations feel redundant — keep both (plan defers the merge, line 70) or unify the grid into the single model surface? **[OPEN-B]** per-task sampler richness — keep temperature-only per task + let Option 2 fill the rest from the file (lean), or also set task-specific min_p/top_p? Both captured in full in the plan's "Phase 1 EXPANSION" section. The MTP/fit part of Phase 1 (the originally-approved scope) is unaffected and still ready to build first.
