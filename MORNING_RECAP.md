...
## ⚠️ ACTIVE DESIGN DISCUSSION (2026-07-03, LATE) — Models-tab / hardware-grid / "one good model" rethink — **NOT DECIDED; RESUME HERE**

> **⛔ This is an UNFINISHED chat-plan design conversation, NOT a decision and NOT built. The user asked me to save it in full detail before compacting, then pick it up. Do NOT treat anything below as settled. Resume by finishing the re-grounding (the one remaining read is the dispatch cascade — see the bottom), then present ONE coherent, self-consistent recommendation, and get an explicit "go" before writing any code. The user's exact standing note: "you are contradicting yourself each time... i think i like your recommendation on changing the grid and 1 good model, but i want you to think about it again before you recommend." So: think it through completely and stop giving off-the-cuff takes.**
...
The EMERGING DIRECTION the user "likes" but has NOT locked (I must re-ground and re-present before it is decided): keep the nine tasks and their per-task settings unchanged; simplify the MODEL layer so that ONE good model that fits your hardware becomes the shared DEFAULT across all tasks (because it is a *good* model, character chat is quick AND effective, not dumb-fast); a per-task model OVERRIDE stays available as an ADVANCED option for measured/big-rig cases — and "advanced" does NOT mean hiding the Tasks tab (the Tasks tab stays exactly as-is, it is the tweak surface; I mis-worded this once and the user rightly flagged it). QuickSetup uses the fit engine to pick + download the single best model for your hardware and set it as the default in one click; the grid/recommendations demote to an optional "browse / compare / see-the-whole-ladder" reference rather than the required front door. Nothing valuable is lost because `coarse_fit` (the grid's brain) carries over into QuickSetup.
...
WHAT REMAINS to finish the re-grounding before I present the single recommendation: read the dispatch cascade end-to-end — `just-llm-runner/llm_runner/llm/preset_resolve.py` (`resolve_task_preset(task_kind) → tks.get(task_kind) or tks.get("")`, the 2-tier task→default) and how the global default preset (`task_kind_presets[""]`) + `/v1/ai/routing`'s `default` interact with the taskKind preset resolution, so the "one default model" maps cleanly onto the existing cascade with no contradiction. THEN write one coherent recommendation covering (a) the grid's fate (retire as primary, keep `coarse_fit` in QuickSetup, optional ladder reference), (b) "Manage all models" (installed-only vs full catalog), (c) the one-good-model default + advanced per-task override, and (d) rewiring QuickSetup to the taskKind/preset model — and get an explicit "go" before any code. NO code is in flight; both repos are clean + synced at runner `b7290ff` / JW `ffdea9a` (the GGUF layer Phases 1–6 shipped; this discussion is a NEW rethink layered on top, not part of that shipped plan).
