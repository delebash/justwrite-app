...
## Current state (2026-07-03) — **GGUF-grounded model layer**: Phases 1–3 SHIPPED (green) · Phase 4 IN PROGRESS (unified "Models" surface)
...
>
> **🔨 PHASE 4 IN PROGRESS (2026-07-03) — the unified "Models" surface.** On resume ("go"), re-read rules + plan + recap, verified state (both repos clean/synced: runner `aac5116`, JW `d03af62`), then GROUNDED in a full read of every touched surface before touching code (rule #7): `recommendations_api.py` (rec store = `(modelId, taskKind, rank, why)`, free-string task_kind), `runner/fit.py::coarse_fit` (the pure fit fn — RAM-gates MoEs via `min_ram_override`), `runner/api.py` (`_fit`/`GET /v1/llm-runner/models`), `runner/schema.py`, `seed.py` (DEFAULT_CATALOG 11 models incl. `nomic-embed-text`; DEFAULT_RECOMMENDATIONS; DEFAULT_TASK_KINDS 9; DEFAULT_SWITCH_PRESETS), `AiModelsArea.vue` (tab host), `RecommendationsEditor.vue`, `ProviderForm.vue` (built-in mounts `LuRunnerEngine` + `LuModelCatalog`), `LuModelCatalog.vue` (owns the flat list + load/unload + Tune modal + Add/Edit-GGUF-inspect), + the research matrix (`model-catalog-research-and-recommendations.md:71-89`). **PLACEMENT DECIDED BY THE USER (2026-07-03):** the unified surface is a **top-level "Models" tab** (chosen over "keep it under Providers → Built-in"). The plan's Phase-4 body was RECONCILED from the superseded "additive" text to unify-now + this placement (plan §"Phase 4 — The unified 'Models' surface", sub-steps 4a–4e; Decision 5 struck; LIVE STATUS flipped to 🔨). **Sub-steps:** 4a runner backend (`DEFAULT_HARDWARE_TIERS` reproducing the research matrix columns · `TASKKIND_FUNCTIONS`/`function_of` map chat/prose/extract/analysis + `other` + `embed` · embed rec row for `nomic-embed-text` · chat re-rank so best-that-fits wins + 9B=faster · pure `build_recommendation_grid()` in new `recommendation_grid.py` + `make_recommendation_grid_router()` in new `recommendation_grid_api.py` → `GET /v1/ai/recommendation-grid`, a read-time VIEW, no schema change · wire in `install.py`) · 4b reuse extraction (`useRunnerModels.js` composable + `TuneMeasureModal.vue` pulled OUT of `LuModelCatalog` so the grid + flat list share ONE model-state — RULE #3, no copy) · 4c the Models tab fold-in (`AiModelsArea` rename Recommendations→Models hosting `RecommendationGrid` + relocated `LuModelCatalog` + `RecommendationsEditor` under an "Advanced" disclosure; `ProviderForm` drops the catalog, keeps the engine panel + a pointer) · 4d JW `seed_presets.py` `p_chat.model` off the 9B → `qwen3.6-35b-a3b-mtp` (best-that-fits AT THE FLOOR; a reseed is required) · 4e quality/faster = display only. **A 3-checker rules panel (architecture-fit · reuse/convergence · grounding) is reviewing the reconciled plan BEFORE any code (T4).** This doc-only commit persists the reconciliation + the user's placement decision for compaction safety; the code lands next.**
