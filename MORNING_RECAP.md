...
**GIT STATE at this save:** runner clean at `4dc12bf` (P1d/P1e code `855abfd` → the box-finding doc commits `fbcdccb`→`c54903d`→`53ca0f7`→`9b61073` → **P1f code + plan AS-BUILT `4dc12bf`**, ALL pushed); JW clean at this recap commit (pushed). Branch `claude/admiring-galileo-il3q0o`. **TASKS:** #29 (anchor) + #113 (SVM P1) in_progress — **P1a–1f DONE + box-verified + shipped (P1f rules-checker FAIL(3)→PASS, ruff clean + 253 pytest), P1g remains** (box-verify router flags on b9644 + the sync-vs-async POST + the child-OOM-log residual); #114 (P2) / #115 (P3) / #116 (P4) / #117 (P5) pending; the model-surface #104–112 is coupled and WAITS on this manager (unblocks at P3). The big.json long-prompt prefill measurement is DONE and recorded in the "LATE" section below + `serving-vram-manager` §8.2b (real 913-token prefill ~131 t/s on the offloaded 35B-A3B; 14B-dense-partial-offload 7.26 t/s validates the §10 speed-floor exclusion).
