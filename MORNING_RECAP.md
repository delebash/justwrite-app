...
- **Hard gates (STRIPPED 2026-07-15, the user's named go):** Stop **Block 0** (re-read rules/recap after compact/clear — a real Read), **Block 1** (a code claim needs evidence tools that turn), **Block 6** (a proposal ends with "SECOND PASS —"). ~~a **commit gate for DELEGATED-agent commits only**~~ — **STRUCK 2026-07-28: there is NO commit gate.** It was retired by the user's order on 2026-07-21 after it hard-denied four commits for which they had EXPLICITLY waived the rules-checker ("no rules checker but you can run tests, go"), leaving verified work unshippable; `~/.claude/hooks/commit-gate.py` is now an exit-0 stub whose docstring carries that record, and the `PreToolUse(Bash)` wiring is gone from `settings.json`. This line survived three days claiming a gate that does not run — verify a gate against its hook file, not against this list. Pre-action is a one-line R1–R5 nudge; plan checks are tiered (1 routine / 3-lens load-bearing); ONE checker per task on the diff. Everything else deleted on lifetime logs — record: claude-config `rules-detail.md` ("THE STRIP") + `EFFECTIVENESS.md`. `test_gates.py` pins ledger refs, escape-fires, and loaded-surface size budgets.
