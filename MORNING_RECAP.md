...
**GIT STATE at this save:** runner clean at `9b61073` (P1d/P1e code `855abfd`, then the box-finding doc commits `fbcdccb`→`c54903d`→`53ca0f7`→`9b61073`, ALL pushed); JW clean at this recap commit (pushed). Branch `claude/admiring-galileo-il3q0o`. **TASKS:** #29 (anchor) + #113 (SVM P1) in_progress — **P1a–1e DONE + box-verified, P1f (fully spec'd, ready to build on "go") / P1g remain**; #114 (P2) / #115 (P3) / #116 (P4) / #117 (P5) pending; the model-surface #104–112 is coupled and WAITS on this manager (unblocks at P3). The big.json long-prompt prefill measurement is DONE and recorded in the "LATE" section below + `serving-vram-manager` §8.2b (real 913-token prefill ~131 t/s on the offloaded 35B-A3B; 14B-dense-partial-offload 7.26 t/s validates the §10 speed-floor exclusion).
