...
## ⛔⛔ SESSION STATE (2026-07-04, EVENING) — READ THIS FIRST after compaction — SERVING/VRAM-MANAGER BUILD IN PROGRESS (Phase 1)
...
**PHASE 1 PROGRESS — P1a–1c DONE + SHIPPED; P1d NEXT.** Committed + pushed on the branch: **runner `26f1ce7`** (rules-checker PASS). What's built (all in `just-llm-runner/llm_runner/runner/process.py`, runtime UNCHANGED because nothing calls the new functions yet — that is 1d):
...
**NEXT — P1d (the RunnerService→router refactor; the biggest/riskiest sub-task).** Per the panel's requirement, START with a per-method strict-diff table of EVERY `RunnerService` method → its router-mode behavior BEFORE editing (the tool that would have caught the T5 omissions). Then: a long-lived `_router` process replaces the single `_runner`, spawned LAZILY on the first `load()` and gated by the SAME engine-present check (`lifecycle.py:469-473` → "engine-not-installed"); `_emit_ini()` renders `<cache_root>/llamacpp/models.ini` from the DB (catalog × `resolve_model_switches` × `compute_fit`, reusing `switches_fn` + `_read_meta`); `load(model_id)` → ensure the section (re-emit if changed) + arbiter (P2) + `POST {router}/models/load {"model":id}`; `stop(model_id)` → `POST /models/unload`; the single-load-in-flight guard (`lifecycle.py:300-301`) becomes per-model; `_state` → a resident-set map and `status()` (`:244`, today reads `_runner.is_alive()` + one modelId `:161-163`/`:487`) reads `GET {router}/models` (re-spec `/v1/llm-runner/status` `api.py:186-188`); `measure()` (`:342-363`) + `tokenize()` (`:365-378`) RE-HOMED onto the router by passing the model id (they use a model-LESS body today `:64`/`:87`); the Lab per-load tuning = the Option-A ephemeral-section re-emit + reload + measure + revert; and a router-level OOM back-off (a child aborting on a too-high `ngl` → re-emit that section at a lower `ngl` + reload, mirroring `_BACKOFF_STEP`, because `start_runner`'s back-off `process.py:379-410` no longer runs in router mode). Then **1e** (add `models_max: int = 2` + `sleep_idle_seconds: int = 900` to `RunnerConfig` `schema.py:115-121`, seed `seed.py:201-204`, read `stores.py:940-942`; OWNERSHIP: DB `models_max` = the cap, the arbiter works within it and owns `load-on-startup`), **1f** (`api.py` resident-set aware: `_status_for` `:99-113` + `/status` read `GET /models`; NEW `GET /v1/llm-runner/resident`), **1g** (verify router flags/endpoints on the pinned b9644 via `llama-server --help` on the user's box; bump `DEFAULT_PINNED_BUILD` `config.py:25` if absent).
...
**GIT STATE at this save:** runner clean at `26f1ce7` (pushed); JW clean at this recap commit (pushed). Branch `claude/admiring-galileo-il3q0o`. **TASKS:** #29 (anchor) + #113 (SVM P1) in_progress; #114 (P2) / #115 (P3) / #116 (P4) / #117 (P5) pending; the model-surface #104–112 is coupled and WAITS on this manager (unblocks at P3). The big.json long-prompt prefill measurement is DONE and recorded in the "LATE" section below + `serving-vram-manager` §8.2b (real 913-token prefill ~131 t/s on the offloaded 35B-A3B; 14B-dense-partial-offload 7.26 t/s validates the §10 speed-floor exclusion).
