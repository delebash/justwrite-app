...
> ## ⛔ MODEL-SURFACE BUILD — APPROVED + IN BUILD (2026-07-05; user "do the plan fold it in and code" → ExitPlanMode approved). PHASE 0 + PHASE 1 DONE + committed; PHASE 2 NEXT.
...
>
> **CURRENT-STATE CORRECTIONS (Explore-verified 2026-07-05 — the design docs were stale):** `LuModelCatalog.vue` is ALREADY a flat visible `<table>` (no "Browse catalog" toggle, no search/sort → Phase 2 ADDS them; actions today = Load/Unload/Download&load/Retry via `/v1/llm-runner/load`+`/stop`). `useCatalogMeta.js` does NOT exist (build `modelApply.js` on `ui/src/composables/useRouting.js` `setDefaultLlm`/`setDefaultEmbedding`). `QuickSetup.vue` embedding line is DISPLAY-ONLY + `CARD_OPTIONS` stops at 24 GB + it can't see `type`; the `?vramMb` "bug" is a NON-issue (already `vram_mb`). The Default/Embedding badge needs a routing`{providerId,model}`↔catalog-`id` join (nothing joins them today).
>
